...
// ── ATV Voice (ATVV) hook ────────────────────────────────────────────────────
//
// Fork addition. The Android TV Voice service has to live on the *same* BLE
// connection and the *same* GATTS application as the HID service — Bluedroid
// keeps a single global GATTS callback, so a second component cannot register
// its own without unhooking this one. This interface lets an external
// component (see `atv_voice`) own the voice service while this component stays
// the only GATTS client: every relevant event is forwarded to the hook.
//
...
class AtvVoiceHook {
 public:
  ...
  /// GATTS app registered — create the voice service's attribute table here.
  virtual void atvv_on_gatts_reg(esp_gatt_if_t gatts_if) = 0;
  /// Attribute table created. Return true if `svc_inst` belonged to the hook.
  virtual bool atvv_on_attr_tab(uint8_t svc_inst, const uint16_t *handles, uint16_t count) = 0;
  virtual void atvv_on_connect(uint16_t conn_id) = 0;
  virtual void atvv_on_disconnect() = 0;
  virtual void atvv_on_mtu(uint16_t mtu) = 0;
  /// Host wrote a characteristic. Return true if the handle was the hook's.
  virtual bool atvv_on_write(uint16_t handle, const uint8_t *value, uint16_t len) = 0;
  virtual void atvv_on_congest(bool congested) = 0;
...
#endif
{
 public:
  ...
  // Fork addition: ATV Voice (ATVV) service, see AtvVoiceHook above.
  void set_atv_voice_hook(AtvVoiceHook *hook) { atv_voice_hook_ = hook; }
  AtvVoiceHook *atv_voice_hook() const { return atv_voice_hook_; }
  ...
  /// Host meldet die Fernbedienung dann dauerhaft als voll.
  void set_battery_level(uint8_t percent);

  /// Verbindungsprofil umschalten, um im Ruhezustand Strom zu sparen.
  /// idle=true : Intervall 120-150 ms, Slave-Latency 3, Supervision 6 s -
  ///   der Funk wacht dann nur noch ~alle 600 ms statt alle 15 ms auf,
  ///   ein Tastendruck geht trotzdem beim naechsten Connection-Event raus.
  /// idle=false: zurueck auf das schnelle HID-Profil (7,5-15 ms, Latency 0).
  /// NICHT waehrend einer Sprachabfrage auf idle stellen.
  void request_conn_profile(bool idle);
 ...
 protected:
  // Fork addition: optional ATV Voice service owner (nullptr without `atv_voice:`).
  AtvVoiceHook *atv_voice_hook_{nullptr};
