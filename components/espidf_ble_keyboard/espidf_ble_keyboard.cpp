...
static const uint8_t hid_report_map[] = {
    ...
    0xC0,              //   End Collection (Physical)
    0xC0,              // End Collection (Application)
#endif  // ESPIDF_BLE_KB_NO_POINTER
};
...
static uint8_t raw_adv_data[] = {
    ...
    // Appearance fuer die Entscheidung "ist das ein Sprach-Zubehoer"
    // heranziehen. 0x0180 = "Generic Remote Control" (Bluetooth SIG
    // Assigned Numbers, Kategorie 6) statt HID-Keyboard-Subkategorie.
    0x03, 0x19, 0x80, 0x01      // Appearance: Generic Remote Control (0x0180)
};
...
// Gesetzt, wenn switch_host() eine bestehende Verbindung getrennt hat. Das
// Werben startet sonst erst im DISCONNECT-Ereignis - bleibt der alte Host
// stumm (z. B. ausgeschaltet), kommt das erst nach dem Supervision-Timeout und
// das Geraet ist bis dahin fuer jeden neuen Host unsichtbar. 0 = nichts offen.
static uint32_t s_pending_adv_after_close_ms = 0;
// Laeuft gerade Werbung? Die Slot-Adresse laesst sich nur setzen, wenn sie
// steht - sonst quittiert der Stack mit "Advertising or scanning now, can't
...
// eine Client-Rolle auf derselben Verbindung und stellt die Anfrage selbst.
// Die MTU gilt pro Verbindung, nicht pro Rolle - ein Erfolg wuerde also auch
// unsere Audio-Notifications groesser machen: statt 7 Paketen pro Block nur
...
static void gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
    ...
    switch (event) {
        case ESP_GATTC_REG_EVT:
            s_gattc_if = gattc_if;
            ESP_LOGI(TAG, "GATTC: client role registered (for MTU request)");
            break;
        case ESP_GATTC_OPEN_EVT:
            if (param->open.status == ESP_GATT_OK) {
                ESP_LOGI(TAG, "GATTC: opened conn %d — requesting larger MTU", param->open.conn_id);
                esp_ble_gattc_send_mtu_req(gattc_if, param->open.conn_id);
            } else {
                ESP_LOGW(TAG, "GATTC: open failed (%d) — MTU stays at default", param->open.status);
            }
            break;
        case ESP_GATTC_CFG_MTU_EVT:
            ESP_LOGI(TAG, "GATTC: MTU negotiated = %u (status %d)", param->cfg_mtu.mtu, param->cfg_mtu.status);
            // Wichtig: Bei einer Client-seitigen Aushandlung feuert das
            // GATTS-MTU-Ereignis NICHT. Ohne diese Weitergabe bliebe der
            // Audiopfad bei den angenommenen 23 Byte und wuerde weiter in
            // 20-Byte-Haeppchen zerlegen - der ganze Gewinn waere verpufft.
            if (param->cfg_mtu.status == ESP_GATT_OK && s_instance && s_instance->atv_voice_hook())
                s_instance->atv_voice_hook()->atvv_on_mtu(param->cfg_mtu.mtu);
            break;
        default:
            break;
    }
...
static void request_host_friendly_conn_params(const esp_bd_addr_t bda) {
    ...
    //   15 ms ohne WLAN: 6.2 Pakete/Fenster -> 413 Notif/s
...
// Verbindungsintervall rausgeht, waehrend die echte Sony-Fernbedienung (siehe
// BLE-Sniff) einen ~3x hoeheren Durchsatz erreicht. Ob der ESP32-Controller
// das ohne diesen Request schon von sich aus tut, ist unklar - kostet nichts,
// es zu versuchen.
static void request_data_length_extension(const esp_bd_addr_t bda) {
    esp_err_t ret = esp_ble_gap_set_pkt_data_len((uint8_t *) bda, 251);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "GAP: Data length extension request failed (%d)", ret);
    }
}
...
// database). Product ID/Version bleiben geraten - dafuer gibt es keine
// oeffentliche Quelle, nur ein BLE-Sniff der Original-Fernbedienung wuerde
// den echten Wert zeigen.
// [Vendor-ID-Source=BT-SIG][Vendor ID=0x00E0][Product ID][Product Version]
static uint8_t  pnp_id_val[7]     = {0x01, 0xE0, 0x00, 0xB2, 0xA1, 0x00, 0x01};
...
// Fork addition: the ATV Voice service, created by the atv_voice component.
#define SVC_INST_ATVV 3
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            // Fork addition: let the ATV Voice service add its own table on the
            // same GATTS interface, so voice lives on the HID connection.
            if (s_instance && s_instance->atv_voice_hook())
                s_instance->atv_voice_hook()->atvv_on_gatts_reg(gatts_if);
            break;
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            ...
            if (svc_id == SVC_INST_DIS) {
                ...
                // Gehoert ebenfalls zur Tastatur (LED-Zustaende vom Host).
            ...
            } else if (s_instance && s_instance->atv_voice_hook()) {
                s_instance->atv_voice_hook()->atvv_on_attr_tab(svc_id, param->add_attr_tab.handles,
                                                               param->add_attr_tab.num_handle);
            }
            break;
        }
        case ESP_GATTS_START_EVT: {
            // Fork addition: only the three core services gate advertising. The
            // ATV Voice service starts on the same interface and is announced
            // separately — a host discovers it after connecting either way, so
            // a problem there must never leave the keyboard unadvertised.
            uint16_t started = param->start.service_handle;
            bool core = started == dis_handle_table[DIS_IDX_SVC] || started == bas_handle_table[BAS_IDX_SVC] ||
                        started == hid_handle_table[IDX_SVC];
            if (!core) {
                ESP_LOGI(TAG, "GATTS: ATV Voice service started");
                break;
            }
            s_services_started++;
        ...
        case ESP_GATTS_CONNECT_EVT: {
            ...
            if (s_instance && s_instance->atv_voice_hook())
                s_instance->atv_voice_hook()->atvv_on_connect(param->connect.conn_id);
            break;
        }
        case ESP_GATTS_DISCONNECT_EVT: {
            ...
            battery_ccc_val = 0;
            if (s_instance && s_instance->atv_voice_hook())
                s_instance->atv_voice_hook()->atvv_on_disconnect();
            do_start_advertising();
            break;
        }
...
void EspidfBleKeyboard::request_conn_profile(bool idle) {
    ...
    if (idle) {
        // Referenz-Firmware: 120-150 ms, Latency 3, 6 s Supervision.
        p.min_int = 0x60;   // 96 * 1.25 ms = 120 ms
        p.max_int = 0x78;   // 120 * 1.25 ms = 150 ms
        p.latency = 3;
        p.timeout = 600;    // 6000 ms
    } else {
        // Schnelles HID-Profil, identisch zu request_host_friendly_conn_params.
        p.min_int = 0x06;   // 7.5 ms
        p.max_int = 0x0C;   // 15 ms
        p.latency = 0;
        p.timeout = 400;    // 4000 ms
    }
...
void EspidfBleKeyboard::set_battery_level(uint8_t percent) {
    ...
    if (battery_level_val == percent) return;
    battery_level_val = percent;
    // Gelesen wird der Wert aus der Attributtabelle, gemeldet wird er nur, wenn
    // der Host den Dienst abonniert hat.
    if (bas_handle_table[BAS_IDX_BAT_VAL] != 0)
        esp_ble_gatts_set_attr_value(bas_handle_table[BAS_IDX_BAT_VAL], 1, &battery_level_val);
    if (is_connected_ && s_gatts_if != ESP_GATT_IF_NONE && (battery_ccc_val & 0x0001))
        esp_ble_gatts_send_indicate(s_gatts_if, conn_id_, bas_handle_table[BAS_IDX_BAT_VAL],
                                    1, &battery_level_val, false);
    ESP_LOGD(TAG, "Akkustand an den Host gemeldet: %u %%", (unsigned) percent);
}
...
void EspidfBleKeyboard::switch_host(uint8_t slot) {
    ...
    if (is_connected_) {
        ...
        // Host nicht mehr antwortet, wird das Werben in loop() erzwungen.
        esp_ble_gatts_close(s_gatts_if, conn_id_);
        s_pending_adv_after_close_ms = millis();
    } else {
        // Nicht verbunden - neu werben. Das Stoppen einer noch laufenden
        // Werbung erledigt do_start_advertising() jetzt selbst und wartet das
        // STOP-Ereignis ab; ein zusaetzliches Stoppen hier wuerde sich damit
        // ueberschneiden.
        do_start_advertising();
    }
}
...
void EspidfBleKeyboard::setup() {
    ...
    // Fernbedienung schickt daraufhin Audio in 203-Byte-Notifications - eine
    // pro Frame statt sieben. Genau dieser Faktor 7 an Paketrate war die
    // Ursache der Dauer-Congestion, die uns ueberhaupt erst zu 8 kHz und
    // damit zur verlustbehafteten Dezimierung gezwungen hat.
    esp_err_t mtu_ret = esp_ble_gatt_set_local_mtu(517);
    if (mtu_ret != ESP_OK) {
        ESP_LOGW(TAG, "Could not raise local MTU (%d) — audio will stay on 20-byte chunks", mtu_ret);
    }
...
void EspidfBleKeyboard::loop() {
    // Werben notfalls selbst starten: nach einem Slot-Wechsel haengt der Start
    // am DISCONNECT-Ereignis des alten Hosts. Ist der ausgeschaltet oder ausser
    // Reichweite, kommt das erst nach dem Supervision-Timeout - bis dahin sieht
    // kein neuer Host das Geraet, und eine Kopplung ist schlicht unmoeglich.
    // 400 statt 1500 ms: der regulaere Trennungsweg meldet sich beim
    // Slot-Wechsel praktisch nie rechtzeitig zurueck, wir warten also auf
    // etwas, das nicht kommt. Bei haeufigem Umschalten zwischen Streamer und
    // Fernseher war die Verzoegerung deutlich spuerbar.
    if (s_pending_adv_after_close_ms != 0 &&
        millis() - s_pending_adv_after_close_ms > 400) {
        s_pending_adv_after_close_ms = 0;
        // Frueher stand hier ein "nur wenn nicht mehr verbunden" - genau der
        // Fall, der sich nie von selbst aufloest, blieb damit ausgespart: Der
        // alte Host antwortet nicht, is_connected_ bleibt stehen, und die
        // Notfrist tat nichts. Jetzt wird die Verbindung auf Controller-Ebene
        // hart getrennt und in jedem Fall geworben.
        ESP_LOGW(TAG, "Kein Disconnect nach Slot-Wechsel - trenne hart und werbe selbst");
        if (is_connected_) {
            esp_ble_gap_disconnect(peer_addr_);
            is_connected_ = false;
            set_paired(false);
        }
        do_start_advertising();
...
void EspidfBleKeyboard::execute_action(const std::string &action) {
    // Fork addition: microphone over the Android TV Voice service. Unlike the
    // "voice" action (which only sends HID Voice Command 0x00CF and leaves the
    // host with no audio source), these drive the ATVV stream itself.
    if (action == "atv_mic_start" || action == "atv_mic_stop") {
        if (atv_voice_hook_ == nullptr) {
            ESP_LOGW(TAG, "Action '%s' needs an `atv_voice:` component — ignored", action.c_str());
            return;
        }
        if (action == "atv_mic_start") atv_voice_hook_->atvv_start_search();
        else atv_voice_hook_->atvv_stop_audio();
        return;
    }
